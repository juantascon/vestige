...
void Initializer::environment() {
    ...
    std::ifstream* video = new std::ifstream();
    
    try {
        video->open(core::Parameters::instance()->VIDEOFILE().c_str(), std::ifstream::in);
        if (video->good()) {
            src = "filesrc location="+core::Parameters::instance()->VIDEOFILE()+" ! decodebin";
            video->close();
        }
    }
